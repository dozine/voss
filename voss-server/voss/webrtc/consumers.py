...
class CallConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if bytes_data:
            ...
            # print(f"Counselor ID: {id}")
            # print(f"Consumer phone: {phone}")
            
            # if CallConsumer.make_db_count == 0:
            #     CallConsumer.make_db_count += 1
            #     call_id = await self.create_voc_record(phone, id)
            #     print("Call id:", call_id)
            #     self.call_id = int(call_id)
            #     print(f"Created VocRecord with id: {self.call_id}")
            # else:
            #     print('Alreay Exist')
    ...
    def read_full_conversation(self):
        ...
        # dir_path = os.path.join(settings.MEDIA_ROOT, "logs", self.id)
        # log_file_path = os.path.join(dir_path, "full_conversation.txt")
        full_conversation = ""
