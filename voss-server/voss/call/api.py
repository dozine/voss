...
def voc_record_delete(request, pk):
    try:
        ...
        # serializer = VocRecordSerializer(voc_record, data=request.data)
        # if serializer.is_valid():
        voc_record.delete()
...
# @api_view(['GET'])
# def voc_record_detail(request, pk):
#     try:
#         voc_record = VocRecord.objects.get(pk=pk)
#         serializer = VocRecordSerializer(voc_record)
...
#         voc_record = VocRecord(user=request.user, customer=customer)
#         voc_record.save()
